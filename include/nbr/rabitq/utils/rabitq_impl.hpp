...
namespace rabitqlib::quant::rabitq_impl {
  ...
  namespace one_bit {
     ...
     */
    template<typename T>
    inline RowMajorArray<T> one_bit_code(const T *data, const T *centroid, size_t dim, int *binary_code) {
     ...
     */
    template<typename T>
    inline void one_bit_code_with_factor(const T *data, const T *centroid, size_t dim, int *binary_code, T &f_add,
                                         T &f_rescale, T &f_error, MetricType metric_type = METRIC_L2) {
      // quantize
      ...
      T tmp_error =
          ...
          std::sqrt((((l2_sqr * l2norm_sqr<T>(xu_cb.data(), dim)) / (ip_resi_xucb * ip_resi_xucb)) - 1) / (dim - 1));
      ...
      if (metric_type == METRIC_L2) {
        ...
        f_add = 1 - dot_product<T>(residual_arr.data(), centroid, dim) + l2_sqr * ip_cent_xucb / ip_resi_xucb;
    ...
    inline void one_bit_compact_code(const T *data, const T *centroid, size_t padded_dim, TC *compact_code, T &f_add,
                                     T &f_recale, T &f_error, MetricType metric_type = METRIC_L2) {
      // binary code
      std::vector<int> binary_code(padded_dim);

      // get binary code
      one_bit_code_with_factor(data, centroid, padded_dim, binary_code.data(), f_add, f_recale, f_error, metric_type);
    ...
    inline void one_bit_compact_codes(const T *data, const T *centroid, size_t num, size_t padded_dim, TC *compact_code,
                                      T *f_add, T *f_rescale, T *f_error, MetricType metric_type = METRIC_L2) {
      constexpr size_t kTypeBits = sizeof(TC) * 8;

#pragma omp parallel for if (Parallel)
      for (size_t i = 0; i < num; ++i) {
        one_bit_compact_code(data + (padded_dim * i), centroid, padded_dim, compact_code + (padded_dim / kTypeBits * i),
  ...
  namespace ex_bits {
    ...
    inline double best_rescale_factor(const T *o_abs, size_t dim, size_t ex_bits) {
      ...
      std::priority_queue<std::pair<double, size_t>, std::vector<std::pair<double, size_t>>, std::greater<>> next_t;
    ...
    template<typename T, typename TP>
    inline T ex_bits_code(const T *residual, size_t dim, size_t ex_bits, TP *ex_code, double t_const = -1) {
    ...
    template<typename T, typename TP>
    inline void ex_bits_code_with_factor(const T *data, const T *centroid, size_t dim, size_t ex_bits, TP *ex_code,
                                         T &f_add_ex, T &f_rescale_ex, T &f_error_ex,
                                         MetricType metric_type = METRIC_L2, double t_const = -1) {
      ConstRowMajorArrayMap<T> data_arr(data, 1, dim);
      ConstRowMajorArrayMap<T> cent_arr(centroid, 1, dim);
      ...
      if (metric_type == METRIC_L2) {
        ...
        f_add_ex = 1 - dot_product<T>(residual_arr.data(), centroid, dim) + l2_sqr * ip_cent_xucb / ip_resi_xucb;
    ...
    template<typename T, typename TP>
    inline void ex_bits_compact_code(const T *data, const T *centroid, size_t padded_dim, size_t ex_bits,
                                     TP *compact_code, T &f_add_ex, T &f_rescale_ex, T &f_error_ex,
                                     MetricType metric_type = METRIC_L2, double t_const = -1) {
      std::vector<uint8_t> ex_code(padded_dim);

      ex_bits_code_with_factor(data, centroid, padded_dim, ex_bits, ex_code.data(), f_add_ex, f_rescale_ex, f_error_ex,
                               metric_type, t_const);
  ...
  namespace total_bits {
    ...
    static inline void rabitq_scalar_impl(
        const T *data, const T *centroid, size_t dim, size_t total_bits, TP *total_code, T &delta, T &vl,
        double t_const = -1, ScalarQuantizerType scalar_quantizer_type = ScalarQuantizerType::RECONSTRUCTION) {
      std::vector<int> binary_code(dim);
      size_t ex_bits = total_bits - 1;

      RowMajorArray<T> residual_arr = rabitq_impl::one_bit::one_bit_code(data, centroid, dim, binary_code.data());
    ...
    template<typename T, typename TP>
    static inline void rabitq_full_impl(const T *data, const T *centroid, size_t dim, size_t total_bits, TP *total_code,
                                        T &f_add, T &f_rescale, T &f_error, MetricType metric_type = METRIC_L2,
                                        double t_const = -1) {
      std::vector<int> binary_code(dim);
      size_t ex_bits = total_bits - 1;
      ...
      if (ex_bits > 0) {
        ex_bits::ex_bits_code_with_factor<T, TP>(data, centroid, dim, ex_bits, total_code, f_add, f_rescale, f_error,
                                                 metric_type, t_const);
      }

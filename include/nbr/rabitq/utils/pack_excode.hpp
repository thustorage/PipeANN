...
namespace rabitqlib::quant::rabitq_impl::ex_bits {
  ...
  inline void packing_3bit_excode(const uint8_t *o_raw, uint8_t *o_compact, size_t dim) {
    ...
    for (size_t d = 0; d < dim; d += 64) {
      ...
      __m128i compact2 =
          _mm_or_si128(_mm_or_si128(vec_00_to_15, vec_16_to_31), _mm_or_si128(vec_32_to_47, vec_48_to_63));
...
// requires AVX512F, similar for remaining functions
  ...
  inline void packing_6bit_excode(const uint8_t *o_raw, uint8_t *o_compact, size_t dim) {
    ...
    for (size_t j = 0; j < dim; j += 16) {
      ...
      *reinterpret_cast<int64_t *>(o_compact) = compact4;
      ...
      int32_t compact2 =
          ((code2_3 & kMask2) << 2) | (code2_2 & kMask2) | ((code2_1 & kMask2) >> 2) | ((code2_0 & kMask2) >> 4);
  ...
  inline void packing_7bit_excode(const uint8_t *o_raw, uint8_t *o_compact, size_t dim) {
    ...
    for (size_t d = 0; d < dim; d += 64) {
      ...
      __m128i compact =
          _mm_or_si128(_mm_and_si128(vec_00_to_15, mask6), _mm_and_si128(_mm_slli_epi16(vec_48_to_63, 6), mask2));
      _mm_storeu_si128(reinterpret_cast<__m128i *>(o_compact), compact);

      compact = _mm_or_si128(_mm_and_si128(vec_16_to_31, mask6), _mm_and_si128(_mm_slli_epi16(vec_48_to_63, 4), mask2));
      _mm_storeu_si128(reinterpret_cast<__m128i *>(o_compact + 16), compact);

      compact = _mm_or_si128(_mm_and_si128(vec_32_to_47, mask6), _mm_and_si128(_mm_slli_epi16(vec_48_to_63, 2), mask2));
   ...
   */
  inline void packing_rabitqplus_code(const uint8_t *o_raw, uint8_t *o_compact, size_t dim, size_t ex_bits) {
    if (ex_bits == 1) {
